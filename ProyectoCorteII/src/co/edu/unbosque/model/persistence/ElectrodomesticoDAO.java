...
/**
 * DAO para manejar los datos de los electrodomésticos. Permite crear, eliminar,
...
public class ElectrodomesticoDAO implements DAO<Electrodomestico> {
	...
	public String showAll() {
		...
		for (int i = 0; i < listaElectrodomestico.size(); i++) {
			sb.append(i + 1).append(" - ").append(listaElectrodomestico.get(i).toString()).append("\n");
		}
		return sb.toString();
	}
	...
	public void leerDesdeArchivoDeTexto(String url) {
		...
		for (String f : fila) {
			...
			temp.setConsumoEnergia(columna[2]);
			temp.setCapacidad(columna[3]);
	...
	public void escribirEnArchivoDeTexto() {
		...
		for (Electrodomestico electrodomestico : listaElectrodomestico) {
			sb.append(electrodomestico.getNombre()).append(";").append(electrodomestico.getPrecio()).append(";")
					.append(electrodomestico.getConsumoEnergia()).append(";").append(electrodomestico.getCapacidad())
					.append(";\n");
		}
